...
| `autobuy.camp.idle_cadence` | **空闲时会话续期的间隔（秒）**，默认 `180`：主程序安静时**不打 search**，只用 Apple 的续期接口（`extendSession`，就是「我仍在购物」那个动作）保住服务端会话。实测一个会话首发 search 8~10s、之后整段（~20 分钟）都是 1~3s——预热打过一发就热了，不用一直发，跟着主程序信号发即可，免得频繁 `checkoutx` 被 541。必须 < `interactionMs`（5 分钟）。点「我仍在购物」对话框、切「我要取货」tab 这些**保活动作不受此间隔限制**，每 15 秒做一次 |
