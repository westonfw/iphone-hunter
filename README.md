...
| `autobuy.camp.idle_cadence` | **冷档**间隔（秒），默认 `240`（4 分钟）：主程序安静时只慢速打一发续会话。**节奏跟着主程序走**——主程序都没看到货，`search` 打了也是白打（结账侧库存不会比 pickup-message 更早有），还白白累积 541 风险。必须 < `interactionMs` 的 5 分钟作废线 |
