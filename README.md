...
| `autobuy.camp.idle_cadence` | **冷档**间隔（秒），默认 `180`（3 分钟）：主程序安静时**不打 search**，只用 Apple 的会话续期接口（`extendSession`，就是「还在吗」对话框上「我还在」按钮点的那个）保活。**节奏跟着主程序走**——主程序都没看到货，`search` 打了也白打（结账侧不会比 pickup-message 更早有），还白白累积 541 风险。必须 < `interactionMs`−`alertMs`（240 秒），否则「还在吗」对话框会先弹出来。每轮还会踹醒页面的客户端计时器、点掉可能弹出的「还在吗」对话框——那个是客户端 JS 倒计时，后台 fetch 重置不了它，不管它会跳去超时页把会话作废 |
