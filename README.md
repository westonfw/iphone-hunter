...
| `autobuy.camp.idle_cadence` | **冷档**间隔（秒），默认 `30`：主程序安静时也一直打 `search`，只是慢些——目的是**把结账会话保持热着**。实测那 10 秒只是一个会话**第一发** `search` 的一次性开销，从第二发起全是 ~500ms（2026-09-21 同一会话 90+ 发，首发 10s、其余 0.5s）。会话凉了下一发又要 10 秒，所以冷档也别停；30 秒一发既保热、又把 `interactionMs` 一直压着不弹「还在吗」，session 内实测不吃 541，还顺带自己发现货。每轮仍会踹醒客户端计时器、点掉「我仍在购物」对话框兜底 |
