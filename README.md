...
> **前言**
> 
> 整个程序都是聊天聊出来的，除了这一小段，全都是AI写的。
> 
> 本以为抢iphone的科技应该不难，结果搞下来的体会就是还是有难度的，搞不定。🤦‍♂️
> 
> 风控应该是和apple id相关的，这也很容易做，毕竟所有的订单都需要绑定账户。但是科技依然能做到2-3s就买完，这就是让我想不通的地方，也是为啥用AI搓一遍的原因。在知道首发时间、大批量放货的时候，手动购买其实也是买的到的，这工具也没啥用。当然，如果有人知道其中的细节，愿意讲讲的，先在此谢过。

盯 Apple 中国官网，**新 iPhone 一放货就把你叫醒**，并在你自己已登录的浏览器里自动走完「加购 → 结账」，只把付款那一下留给你。

> **现状：实际抢不到手机。**
>
> 监控和提醒是好用的，但从「有货」到「订单提交」，光 Apple 自己的加购 → 结账 → 下单流程就要几十秒
> （加购到结账页就占了约 19 秒，见[实测耗时](#实测耗时)），这段时间耗在 Apple 服务端，客户端怎么优化都压不动；
> 而热门机型的补货窗口常常只有几十秒甚至更短，等流程走完货早就没了。
>
> 所以这个仓库现在的价值是**记录**：Apple 中国官网网页版的库存查询、加购、结账六步是怎么走的，
> 有哪些坑（见[实测发现汇总](#实测发现汇总)）。适合用来学习研究，别指望它帮你买到手机。
...
| `autobuy.proxy` | 结账走的**固定 IP** 代理，如 `http://代理服务器IP:8081`，留空直连。配了之后 `hunter connect --launch` 起的 Chrome 会带一段 PAC：只有 Apple 的域（apple.com.cn / apple.com / icloud.com.cn / cdn-apple.com / mzstatic.com）走这个代理，其余直连。**必须是固定 IP 的代理**，结账会话被 Akamai 绑在 IP 上，中途换 IP 就作废。Chrome 的代理设置带不了密码，代理那边得按来源 IP 放行买手机器。挂到别人起的 Chrome 时改不了代理，程序会读 chrome://version 核对启动参数，没带就在日志里喊。被 541 之后照常静默冷却 |
