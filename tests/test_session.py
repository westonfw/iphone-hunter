...
class StopRetryingWhenBlockedTests(unittest.TestCase):
    """被限流时「再换一个地址试试」是最糟的反应：拦截在边缘层，对所有 secureN
    一视同仁，多试一次就把退避撞深一次。"""

    class FakePage:
        def __init__(self, evaluate_result):
            self.url = "https://www.apple.com.cn/shop/bag"
        ...
        def goto(self, url, **kw):
            ...
            # 落到结账页，免得 on_checkout 每个候选都空等 9s 超时
            self.url = "https://secure7.www.apple.com.cn/shop/checkout?_s=Fulfillment-init"
    ...
    def _placer(self):
        ...
        return OrderPlacer(log=lambda *a: None)

    def test_blocked_bagx_stops_all_further_attempts(self):
        page = self.FakePage({"ok": False, "blocked": 541, "tried": []})
        placer = self._placer()
        placer.enter(self.FakeCtx(page), page)
        # 一个候选地址都不该再试
        self.assertEqual([], page.gotos)
        self.assertTrue(placer.blocked.get("hits"))
        self.assertEqual(541, placer.blocked["hits"][0]["status"])

    def test_unblocked_still_tries_candidates(self):
        """没被拦时行为不变——别把退避改成了「永远不试」。"""
        # text 是给 find_continue/on_checkout 用的：认出这是 Fulfillment 步，
        # 就不会每个候选都空等 9s 超时
        page = self.FakePage({"ok": False, "goto": "", "tried": [],
                              "text": "继续填写取货详情"})
        placer = self._placer()
        placer.enter(self.FakeCtx(page), page)
        self.assertGreater(len(page.gotos), 0)

    def test_aos_post_js_aborts_on_block_codes(self):
        from hunter.checkout import JS_AOS_POST
        for code in ("541", "503", "429"):
            self.assertIn(code, JS_AOS_POST)
        self.assertIn("blocked", JS_AOS_POST)
