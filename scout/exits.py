...
class ExitPool:
    ...
    def _make(self, who: str, proxy: str | None, now: float) -> Exit:
        ...
        return Exit(id=who, proxy=proxy, client=self._client(proxy), pacer=pacer,
                    due_at=self._spread(now, pacer), seen_at=now)
    ...
    def describe(self) -> str:
        with self._lock:
            bits = [f"{e.id}"
                    f"{'(直连)' if e.direct else '(代理)' if e.configured else ''}"
