...
#:
#: 为什么不再是固定 30 秒：那是为「多出口时晾坏的那条、用好的顶上」定的。可只配
#: **单条代理 + use_direct=false** 时，晾 30 秒 = 主程序全盲 30 秒，没有别的出口顶；
#: 而代理一次失败大多是瞬时的（抖一下），不该按「代理彻底死了」罚 30 秒。所以第一次
#: 只晾 3 秒快速探恢复，真挂了才逐步拉长。
#:
